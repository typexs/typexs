...
import { map , clone} from '@typexs/generic';
...
class SqlSchemaTypesSpec {
  ...
  async 'date type with created and updated variant'() {
    ...
    expect(map(tables, t => t.name)).to.have.include.members(['date_type']);

    const cols_condition_holder = await c.connection.query('PRAGMA table_info(\'date_type\')');
    expect(map(cols_condition_holder, t => t.name)).to.have.members([
      'id',
      'date',
      'created_at',
      'updated_at'
    ]);
    expect(map(cols_condition_holder, t => t.dflt_value)).to.have.members([
      null,
      null,
      'datetime(\'now\')',
      'datetime(\'now\')'
    ...
    const _clone = clone(test_save_01);
    ...
    expect(_clone.created_at).to.be.eq(test_save_02.created_at);
    expect(_clone.updated_at).to.be.lessThan(test_save_02.updated_at);
