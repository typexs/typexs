...
import { redis_host, redis_port } from '../../../../../base/test/functional/config';
...
(async function() {
  ...
  let bootstrap: Bootstrap = getBootstrapForSpawn('node_tasks', {
    app: { path: __dirname },
    eventbus: {
      default: <IEventBusConfiguration>{ adapter: 'redis', extra: {
    ...
    modules: TestHelper.modulSettings(['base', 'server', 'tasks']),
    tasks: {
