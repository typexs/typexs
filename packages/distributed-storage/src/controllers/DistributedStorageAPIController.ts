import { assign, isArray, isBoolean, isEmpty, isFunction, isNumber, isPlainObject, isString, map } from '@typexs/generic';
...
import { C_API, XS_P_$LABEL, XS_P_$URL } from '@typexs/server/libs/Constants';
...
import { ContextGroup } from '@typexs/server/decorators/ContextGroup';
import { Access } from '@typexs/server/decorators/Access';
import { DistributedStorageEntityController } from '../lib/DistributedStorageEntityController';
import {
...
import { IDistributedFindOptions } from '../lib/find/IDistributedFindOptions';
import { IDistributedAggregateOptions } from '../lib/aggregate/IDistributedAggregateOptions';
import { IDistributedSaveOptions } from '../lib/save/IDistributedSaveOptions';
import { IDistributedUpdateOptions } from '../lib/update/IDistributedUpdateOptions';
import { IDistributedRemoveOptions } from '../lib/remove/IDistributedRemoveOptions';
...
export class DistributedStorageAPIController {
  ...
  static _afterEntity(entityRef: IEntityRef | IEntityRef[], entity: any[]): void {
    ...
    //   e[XS_P_$URL] = url;
    //   e[XS_P_$LABEL] = isFunction(e.label) ? e.label() : map(props, p => p.get(e)).join(' ');
    // });
  ...
  private prepareEntities(entityDef: IEntityRef, data: any, options: IDistributedSaveOptions = {}) {
    ...
    let entities;
    if (isArray(data)) {
      entities = map(data, d => entityDef.build(d, buildOpts));
    } else {
